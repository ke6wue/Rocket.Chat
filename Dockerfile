...
# IMPORTANT: use npm here, not yarn. This directory is a self-contained
# bundle with its own package.json / npm-shrinkwrap.json generated by
# Meteor's build — it is NOT a Yarn workspace. Rocket.Chat's own official
# Dockerfile uses plain `npm install` at this exact step for that reason.
# Yarn Berry (this repo's package manager everywhere else) defaults to a
# different module resolution strategy than a flat node_modules folder,
# which was producing an incomplete install here and breaking Meteor's
# internal module loader hook at runtime ("Cannot find module
# '/base-runtime/...'").
